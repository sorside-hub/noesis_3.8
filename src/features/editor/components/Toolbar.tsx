...
import { 
  Undo, Redo,
  Bold, Italic, Highlighter, Heading as HeadingIcon, List, CheckSquare,
  ListOrdered, Strikethrough, TextQuote, Code, SquareTerminal,
  Link, Minus, Brackets, Info
} from 'lucide-react';
...
export const Toolbar = () => {
  ...
  useEffect(() => {
    ...
    const checkKeyboardAndOffset = () => {
      ...
      if (vv) {
        // Calculate exact distance from layout bottom to visual viewport bottom (iOS Safari / PWA)
        const layoutHeight = window.innerHeight;
        const visualBottom = vv.height + vv.offsetTop;
        const rawOffset = layoutHeight - visualBottom;

        // If rawOffset is tiny (<= 2px) or negative, snap to 0 to avoid subpixel gaps
      ...
      // Android Chrome (interactive-widget / window resize):
      const heightDifference = maxKnownHeight - currentHeight;
      if (heightDifference > 100) {
        keyboardDetected = true;
      }

      // Non-touch desktop fallback during responsive testing
  ...
  const handleCallout = () => {
    ...
    if (selectedText) {
      ...
      view.dispatch({
        ...
        selection: {
          anchor: selection.from + 10 + indented.length
        }
      });
    } else {
      view.dispatch({
  ...
  const tools = [
    { icon: <Undo size={18} />, label: 'Undo (Ctrl+Z)', action: handleUndo },
    { icon: <Redo size={18} />, label: 'Redo (Ctrl+Y)', action: handleRedo },
    { icon: <Bold size={18} />, label: 'Bold', action: () => wrapSelection('**', '**') },
    { icon: <Italic size={18} />, label: 'Italic', action: () => wrapSelection('*', '*') },
    { icon: <Highlighter size={18} />, label: 'Highlight', action: () => wrapSelection('==', '==') },
    ...
    { icon: <Info size={18} />, label: 'Callout Box', action: handleCallout },
