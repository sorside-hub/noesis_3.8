...
export const PreviewPane = forwardRef<PreviewPaneRef, PreviewPaneProps>(({ title, content, nodes, onDoubleClick, onChange, onWikilinkClick }, ref) => {
  ...
  const handleClick = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    // 3. Handle Foldable Heading Click
    const heading = target.closest('.markdown-heading') as HTMLElement | null;
    if (heading) {
      if (target.closest('a')) return; // Allow links inside headings to navigate normally

      e.stopPropagation();
      const depth = parseInt(heading.getAttribute('data-heading-depth') || '1', 10);
      ...
      // Traverse next sibling DOM elements until next heading of same or higher level
      let sibling = heading.nextElementSibling as HTMLElement | null;
        while (sibling) {
          const siblingDepthAttr = sibling.getAttribute('data-heading-depth');
          if (siblingDepthAttr) {
            const siblingDepth = parseInt(siblingDepthAttr, 10);
