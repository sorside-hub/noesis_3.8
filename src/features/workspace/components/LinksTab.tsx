            ...
            {backlinks.map((node) => (
              ...
              >
                <div className="flex items-center gap-2 truncate min-w-0">
                  <FileText size={14} className="text-text-muted shrink-0 group-hover:text-text-primary" />
                  <span className="text-xs font-medium text-text-primary group-hover:text-text-heading truncate">
                    {node.name}
                  </span>
                </div>
  ...
  return (
    <div className="space-y-3 animate-in fade-in duration-150">
      ...
      {outgoingLinks.length === 0 ? (
        <div className="py-12 text-center text-xs text-text-muted">
          Tidak ada tautan wiki-link <span className="font-mono text-text-primary bg-bg-hover px-1 py-0.5 rounded border border-border-subtle">[[...]]</span> ditemukan di catatan ini.
        </div>
      ) : (
        <div className="space-y-1.5">
          {outgoingLinks.map((item, idx) => (
            ...
            >
              <div className="flex items-center gap-2 truncate min-w-0">
                <Link2 size={14} className="text-text-muted shrink-0 group-hover:text-text-primary" />
                <span className="text-xs font-medium text-text-primary truncate group-hover:text-text-heading">
                  {item.targetName}
                </span>
              </div>
              {item.matchedNode ? (
