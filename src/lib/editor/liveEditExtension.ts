...
function getLiveEditDecorations(view: EditorView, isSourceMode: boolean) {
    ...
    syntaxTree(view.state).iterate({
      ...
      enter: (node) => {
        ...
        if (type === 'StrongEmphasis') {
          ...
          const bqText = view.state.doc.sliceString(node.from, node.to);
          const calloutHeaderMatch = bqText.match(/^>\s*\[!([a-zA-Z0-9_-]+)\]/);
          if (calloutHeaderMatch) {
            const typeKey = calloutHeaderMatch[1].toLowerCase();
            let calloutBorder = 'border-l-4 border-blue-500/80 bg-blue-500/10 dark:bg-blue-500/15';
            if (['tip', 'hint'].includes(typeKey)) {
              calloutBorder = 'border-l-4 border-emerald-500/80 bg-emerald-500/10 dark:bg-emerald-500/15';
            } else if (['warning', 'caution'].includes(typeKey)) {
              calloutBorder = 'border-l-4 border-amber-500/80 bg-amber-500/10 dark:bg-amber-500/15';
            } else if (['danger', 'error', 'bug'].includes(typeKey)) {
              calloutBorder = 'border-l-4 border-rose-500/80 bg-rose-500/10 dark:bg-rose-500/15';
            } else if (['success', 'check', 'done'].includes(typeKey)) {
              calloutBorder = 'border-l-4 border-emerald-500/80 bg-emerald-500/10 dark:bg-emerald-500/15';
            } else if (['important'].includes(typeKey)) {
              calloutBorder = 'border-l-4 border-purple-500/80 bg-purple-500/10 dark:bg-purple-500/15';
            } else if (['question', 'faq', 'help'].includes(typeKey)) {
              calloutBorder = 'border-l-4 border-indigo-500/80 bg-indigo-500/10 dark:bg-indigo-500/15';
            }
            decorations.push(Decoration.mark({ class: `${calloutBorder} pl-3.5 pr-2 py-1 my-1.5 rounded-r-lg block font-normal text-text-primary not-italic` }).range(node.from, node.to));
          } else {
            decorations.push(Decoration.mark({ class: 'border-l-4 border-border-default pl-4 italic text-text-quote block my-1.5' }).range(node.from, node.to));
          }
        } else if (type === 'Link') {
  ...
  // Highlight ==text== regex decorations in visible ranges
  ...
  for (let { from, to } of view.visibleRanges) {
    ...
    while ((match = highlightRegex.exec(text)) !== null) {
      ...
      const isCursorInside = view.hasFocus && selection.head >= matchFrom && selection.head <= matchTo;

      if (isSourceMode) {
        decorations.push(Decoration.mark({ class: 'text-text-disabled font-mono' }).range(matchFrom, contentFrom));
        decorations.push(Decoration.mark({ class: 'bg-amber-300/35 dark:bg-amber-400/25 px-0.5 rounded' }).range(contentFrom, contentTo));
        decorations.push(Decoration.mark({ class: 'text-text-disabled font-mono' }).range(contentTo, matchTo));
      } else {
        if (!isCursorInside) {
          decorations.push(hideMarkDecoration.range(matchFrom, contentFrom));
          decorations.push(Decoration.mark({ class: 'bg-amber-300/35 dark:bg-amber-400/25 px-0.5 rounded' }).range(contentFrom, contentTo));
          decorations.push(hideMarkDecoration.range(contentTo, matchTo));
        } else {
          decorations.push(Decoration.mark({ class: 'text-text-disabled font-mono' }).range(matchFrom, contentFrom));
          decorations.push(Decoration.mark({ class: 'bg-amber-300/35 dark:bg-amber-400/25 px-0.5 rounded' }).range(contentFrom, contentTo));
          decorations.push(Decoration.mark({ class: 'text-text-disabled font-mono' }).range(contentTo, matchTo));
